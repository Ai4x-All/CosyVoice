...
def batch(tts_type, outname, params):
    ...
    if tts_type == 'tts':
        ...
        # 仅文字合成语音
        for _, j in enumerate(sft_model.inference_sft(text, params['role'], stream=False, speed=params['speed'])):
        ...
        for _, j in enumerate(tts_model.inference_zero_shot(text, params.get('reference_text'), prompt_speech_16k, stream=False, speed=params['speed'])):
            audio_list.append(j['tts_speech'])
        ...
        for _, j in enumerate(tts_model.inference_cross_lingual(text, prompt_speech_16k, stream=False, speed=params['speed'])):
            audio_list.append(j['tts_speech'])
...
def audio_speech():
    ...
    if voice in VOICE_LIST:
        ...
        return jsonify({"error": {"message": "必须填写配音角色名或参考音频路径", "type": "ValueError", "param": f'speed={speed},voice={voice},input={text}', "code": 400}}), 500
